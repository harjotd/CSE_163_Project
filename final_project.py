...
def parse(file_1, file_2):
    ...
    unique_id = dataset['album_id'].unique()
    album_pop = dict()
    for i in range(0, len(unique_id)):
        album_pop[unique_id[i]] = sp.album(album_id=unique_id[i])['popularity']
        if i % 1000 == 0:
            print("RUNNING")
            time.sleep(10)
    complete_df = pd.DataFrame(list(album_pop.items()),
                               columns=['album_id', 'popularity'])
    dataset = dataset.merge(complete_df, left_on='album_id',
                            right_on='album_id', how='inner')
    dataset.to_csv('mainset.csv')
    return dataset


def dance_vs_pop(data):
    """Takes in the complete dataset as a parameter, and prints out the
    correlation coefficient between popularity and danceability, while
    also plotting a graph."""
    mean = data.groupby('album')['danceability'].mean()
    dance_v_pop = pd.DataFrame({'album': mean.index,
                               'album_danceability': mean.values})
    dance_v_pop = data.merge(dance_v_pop, left_on='album', right_on="album",
                             how='right')
    dance_v_pop = dance_v_pop[['album', 'album_danceability',
                               'popularity']].drop_duplicates()
    sns.regplot(data=dance_v_pop, x='album_danceability', y='popularity')
    mplot.show()
    correlation = np.corrcoef(dance_v_pop['album_danceability'],
                              dance_v_pop['popularity'])
...
def unique_artists(data):
    ...
    per_year.plot(x='Year', y='Count', kind='line')
    mplot.show()
    mplot.savefig("Unique Artists.png")
