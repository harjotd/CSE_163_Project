...
ACOUSTICS = 'Datasets/acoustics.csv'
BILLBOARD = 'Datasets/billboard.csv'
...
def problem1(data):
    ...
    newdata = pd.DataFrame({'album': data1.index, 'album_danceability': data1.values})
    ...
    i = i[['album_danceability', 'popularity']].drop_duplicates()
...
def popularity(data):
    ...
    mplot.savefig("Most Popular Day(M-S)")
    ...
    mplot.savefig('Most Popular Day(1-31)')
    ...
    mplot.savefig("Most Popular Month")
    ...
    mplot.savefig("Most Popular Season")
...
def unique_artists(data):
    ...
    Top 200.'''
    year = pd.to_datetime(data['list_date']).dt.year
    data['year'] = year
    ...
    sns.lineplot(x=per_year['Year'], y=per_year['Count'])
    mplot.savefig("Unique Artists")
...
def top_10(data):
    ...
    top_10_rank = top_10_rank[['album', 'rank']]
    ...
    highest_ranked = pd.DataFrame({'Album': highest_ranked.index,
                                  'Count': highest_ranked.values})
    highest_ranked = highest_ranked.head(10)
    ax = sns.barplot(x='Album', y='Count', data=highest_ranked)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=40, ha="right")
    mplot.tight_layout()
    mplot.savefig("Most Frequent Top 10 Albums")
